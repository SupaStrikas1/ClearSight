...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      console.log("NAFNet cwd:", nafnetCwd);
      console.log(
        "Full command:",
      ...
      await exec(
        `${pythonPath} "${path.join(nafnetCwd, "basicsr/demo.py")}" -opt "${path.join(nafnetCwd, "options/test/SIDD/NAFNet-width32.yml")}" --input_path "${tempInputPath}" --output_path "${tempOutputPath}"`,
        {
          ...
          env: {
            ...
            PYTHONPATH: nafnetCwd, // ← THIS LINE ADDS THE ENTIRE NAFNET FOLDER TO IMPORT PATH
          },
        },
      );

      // Check output exists
      if (!(await fs.stat(tempOutputPath)).isFile()) {
        throw new Error("No output file generated by NAFNet");
      }
